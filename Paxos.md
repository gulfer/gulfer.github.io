...
double stableIntervalMicros		生成令牌的间隔时间（毫秒）
double nextFreeTicketMicros		上一个令牌的生成时间（毫秒）
```
...
```
在设置QPS前，需要调用resync方法对当前毫秒值进行令牌添加，这里的入参即为SleepingStopwatch当前已阻塞的毫秒数。resync方法的逻辑是：如果当前已阻塞时间大于上一次获取令牌的时间，将已存储的令牌增加（当前阻塞时间 - 距上一次令牌生成的时间）/令牌生成间隔，比如如果阻塞时间已达到1000毫秒，上一次令牌生成时的阻塞时间为500毫秒，令牌生成间隔为100毫秒（QPS=10），则新生成5个令牌。最后再将上次令牌生成时间更新为当前阻塞时间。resync不仅在setRate时用到，在调用aquire方法获取令牌时也会用到，也就是说令牌的生成其实是在变更QPS设定和获取令牌时进行的，这样的好处在于不需要单独维护线程去生成令牌。

补充完令牌后，再设置生成令牌的间隔时间，公式就是1秒/QPS，再转成毫秒数。而当前存储的令牌数和最大存储令牌数的计算方式则继续通过抽象方法由子类实现。SmoothBursty中的计算方式为，最大存储令牌数为爆发时间（上面提到的默认1秒）*QPS，当前存储的令牌数在初始化时为0.0。
...
总结一下，本文通过对RateLimiter代码进行简单的解析，介绍了令牌桶的基本实现原理，同时比较了流控与Hystrix熔断的差别。RateLimiter适用于处理处理存在突发流量暴涨的限流场景，但由于并不是企业级产品，在分布式环境下，还是需要基于分布式架构设计的工具，比如redis来实现流控。说点题外话，这次去学习RateLimiter，还是在项目中遇到了类似的问题，想借鉴其设计思路，读代码还是要带着问题去读，要参照某个场景去思考。另外就是建议先理解原理再读代码，明白设计上的理由，解决了什么问题，有了基本的思路之后再去扣细节，会有助于代码的理解。
